...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error || "มีข้อผิดพลาดเกิดขึ้น")
      } else {
        router.push("/login")
      }
    } catch (err) {
  ...
  const getPasswordStrength = () => {
    ...
    if (strength <= 1) return { strength: 1, label: "อ่อนแอ", color: "bg-rose-500" }
    if (strength <= 2) return { strength: 2, label: "ปานกลาง", color: "bg-amber-500" }
    if (strength <= 3) return { strength: 3, label: "ดี", color: "bg-blue-500" }
    return { strength: 4, label: "แข็งแกร่ง", color: "bg-emerald-500" }
  }
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 font-light">สร้างบัญชีเพื่อเริ่มต้นใช้งาน</p>
        </div>
              ...
              {password && (
                <div className="mt-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-slate-400">ความแข็งแกร่ง:</span>
                ...
                <div className="mt-2 flex items-center gap-2">
                  {password === confirmPassword ? (
                    <>
                      ...
                      <span className="text-xs text-emerald-400 font-medium">รหัสผ่านตรงกัน</span>
                    </>
                  ) : (
                    <>
                      ...
                      <span className="text-xs text-rose-400 font-medium">รหัสผ่านไม่ตรงกัน</span>
                    </>
                  )}
                </div>
              )}
            ...
            <div className="pt-2">
              <label className="flex items-start gap-3 cursor-pointer group">
                <input
                  type="checkbox"
                  required
                  className="w-4 h-4 mt-0.5 rounded border-2 border-slate-600 bg-slate-900/50 checked:bg-amber-500 checked:border-amber-500 transition-all cursor-pointer flex-shrink-0"
                />
                <span className="text-sm text-slate-400 group-hover:text-slate-300 transition-colors">
                  ฉันยอมรับ{" "}
                  <Link href="/terms" className="text-amber-400 hover:text-amber-300 font-medium">
                    ข้อกำหนดการใช้งาน
                  </Link>
                  {" "}และ{" "}
        ...
        <div className="mt-8 text-center">
          <p className="text-slate-500 text-sm font-light">
            เข้าร่วมกับผู้ใช้งานหลายพันคนทั่วโลก
          </p>
        </div>
      </div>
    </main>
